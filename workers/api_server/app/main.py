...
async def _execute(action: str, payload: dict):
    return await asyncio.to_thread(service.execute, action, payload)
...
async def login(payload: LoginRequest):
    try:
        ...
        result = await _execute("login", payload.model_dump(exclude_none=True))
...
async def logout(payload: LogoutRequest):
    try:
        ...
        result = await _execute("logout", payload.model_dump(exclude_none=True))
        return ApiActionResponse(success=result.get("success", False), data=result)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
...
async def signals_latest():
    try:
        ...
        result = await _execute("get_signals", {})
        return ApiActionResponse(success=result.get("success", False), data=result)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
...
async def room_status(payload: RoomStatusRequest):
    try:
        ...
        result = await _execute("check_room_status", payload.model_dump())
...
async def portfolio_rule(payload: PortfolioRuleRequest):
    result = await _execute("portfolio_rule_upsert", payload.model_dump())
    return ApiActionResponse(success=result.get("success", False), data=result)


@app.post("/portfolio/orders", response_model=ApiActionResponse)
async def portfolio_order(payload: PortfolioOrderRequest):
    result = await _execute("portfolio_order_create", payload.model_dump(exclude_none=True))
    return ApiActionResponse(success=result.get("success", False), data=result)
...
async def portfolio_db_records():
    result = await _execute("portfolio_db_records", {})
    return ApiActionResponse(success=result.get("success", False), data=result)
...
async def portfolio_strategy_pnl_positions(strategy: str):
    result = await _execute("portfolio_strategy_pnl_positions", {"strategy": strategy})
    return ApiActionResponse(success=result.get("success", False), data=result)
...
async def admin_db_all(x_admin_key: str = Header(default="", alias="x-admin-key")):
    ...
    result = await _execute("portfolio_admin_db", {})
    return ApiActionResponse(success=result.get("success", False), data=result)
...
async def room_action(action_name: str, payload: RoomActionRequest):
    ...
    try:
        ...
        result = await _execute("room_action", body)
        return ApiActionResponse(success=result.get("success", False), data=result)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
