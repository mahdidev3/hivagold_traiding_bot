...
class TradingWorkerService:
    ...
    async def _create_position_for_test_strategy(self, bot: BotThreadConfig) -> Optional[dict[str, Any]]:
        ...
        simulator_url = bot.metadata.get("simulator_url", "http://bot-simulator-worker-service:8007")
        url = f"{simulator_url.rstrip('/')}/simulator/position/create"
        body = {"task_id": task_id, **payload}
        result = await asyncio.to_thread(requests.post, url, json=body, timeout=15)
    ...
    async def _run_bot(self, bot: BotThreadConfig):
        ...
        if session:
            await self._append_log(bot.task_id or "", "info", "session loaded for task", {"has_cookies": bool(session.get('cookies'))})

        if strategy.name == "simple_position_test_v1":
            try:
                created = await self._create_position_for_test_strategy(bot)
                await self._append_log(bot.task_id or "", "info", "position created", {"response": created})
            except Exception as exc:
                await self._append_log(bot.task_id or "", "error", "failed to create position", {"error": str(exc)})
